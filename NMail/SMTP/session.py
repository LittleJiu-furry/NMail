...
class SMTPSession:
    def __init__(self, server: "baseSMTPService", reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        self.reader = reader
        self.writer = writer
        self.server = server
    ...
    async def callEmit(self, event_type:str, *args, **kwargs):
        # 是否有对应的事件处理函数
        if not sessionEvent.hasEvent(event_type):
            logger.warning(f"Client({self.getPeerName()}) send invalid command: {event_type}")
            await self.send(500, "Invalid command")
            return
        sessionModule = SessionModel(self)
        sessionEvent.emit(event_type, sessionModule, *args, **kwargs)
    ...
    async def service(self):
        async for data in self.__GetData():
            await asyncio.sleep(0)
            if not data:
                logger.warning(f"Client({self.getPeerName()}) session get none data, closing session")
            ...
            if (command == SMTPCommand.HELO):
                # 处理HELO命令
                if args is None:
                    logger.warning(f"Client({self.getPeerName()}) send invalid HELO command: {data}")
            ...
            if (command == SMTPCommand.EHLO):
                # 处理EHLO命令
                if args is None:
                    logger.warning(f"Client({self.getPeerName()}) send invalid EHLO command: {data}")
            ...
            if (command == SMTPCommand.MAIL):
                # 处理MAIL命令
                if args is None:
                    logger.warning(f"Client({self.getPeerName()}) send invalid MAIL command: {data}")
            ...
            if (command == SMTPCommand.RCPT):
                # 处理RCPT命令
                if args is None:
                    logger.warning(f"Client({self.getPeerName()}) send invalid RCPT command: {data}")
            ...
            if (command == SMTPCommand.VRFY):
                # 处理VRFY命令
                if args is None:
                    logger.warning(f"Client({self.getPeerName()}) send invalid VRFY command: {data}")
            ...
            if (command == SMTPCommand.EXPN):
                # 处理EXPN命令
                if args is None:
                    logger.warning(f"Client({self.getPeerName()}) send invalid EXPN command: {data}")
            ...
            if (command == SMTPCommand.HELP):
                # 处理HELP命令
                if args is None:
                    logger.warning(f"Client({self.getPeerName()}) send invalid HELP command: {data}")
            ...
            if (command == SMTPCommand.ETRN):
                # 处理ETRN命令
                if args is None:
                    logger.warning(f"Client({self.getPeerName()}) send invalid ETRN command: {data}")
            ...
            if (command == SMTPCommand.BDAT):
                # 处理BDAT命令
                if args is None:
                    logger.warning(f"Client({self.getPeerName()}) send invalid BDAT command: {data}")
